...
// Generated 31-May-2021 01:01:13
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ghostfollower\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ghostfollower_M, (NULL));
        ghostfollower_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ghostfollower_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ghostfollower_M)
                == (NULL)) || rtmGetStopRequested(ghostfollower_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ghostfollower_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
